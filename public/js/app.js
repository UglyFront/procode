...
function mail() {
        ...
        if ((body.name && body.phone && body.email && body.city) != "") {
            fetch(`http://localhost:2222/mail?name=${body.name}&phone=${body.phone}&email=${body.email}&city=${body.city}`)
...
cbBtn.onclick = () => {
    if ((cbName.value && cbPhone.value && cbQuestion.value) != "") {
        fetch(`http://localhost:2222/cb?n=${cbName.value}&p=${cbPhone.value}&q=${cbQuestion.value}`)
        .then(alert("Мы свяжемся с Вами в ближайшее время!"))
    }
    else {
        alert("Форма пуста...")
